...
We introduce \textbf{Recursive Aesthetic Reinforcement (RAR)}: a four-stage loop in which (1) models produce stereotyped outputs, (2) humans recognize them as representative, (3) humans redistribute them, and (4) the redistribution is reingested into the next training corpus through curation pipelines (aesthetic-score filters, popularity ranking, RLHF reward data). Each cycle narrows the representational distribution.

A pilot (\(n=20\) DALL$\cdot$E~3 images, \(n=17\) participants) supports stages 1--2: every leadership prompt produced male-coded, white-presenting outputs (100\%); participants recognized DALL$\cdot$E defaults as ``representative'' 71\% of the time for CEO (\(p=.0001\), OR\,=\,7.20, surviving Bonferroni). The effect is demographically contingent --- male participants chose DALL$\cdot$E defaults 3$\times$ as often as female participants (Cohen's \(d=1.48\)). The loop runs, but not for everyone, and that asymmetry is the most actionable finding.

We then operationalize stage 4 against \textbf{LAION-Aesthetics V2} --- a deployed instance of human-rated $\to$ predictor-filtered $\to$ model-trained that produced Stable Diffusion's training corpus. The methodology, executable analysis pipeline, and pre-registered hypotheses are open-sourced; this paper reports the framework and the path to closure.
...
\begin{itemize}\itemsep0pt
  \item \textbf{Author 1} --- Conceptualization (lead); Formal analysis (lead, pilot statistics); Methodology (lead, RAR framework, Stage-4 LAION audit design); Software (LAION analysis pipeline); Visualization (RAR loop diagram); Writing --- original draft (lead); Writing --- review \& editing.
  \item \textbf{Author 2} --- Conceptualization (supporting); Investigation (lead, pilot survey design and recruitment); Data curation (lead, pilot stimulus generation and qualitative coding); Formal analysis (supporting); Writing --- review \& editing.
  \item \textbf{Author 3} --- [TODO: confirm role with Antardip --- candidate roles include Methodology (Stage-4 audit), Software (LAION pipeline), Validation (replication design), Writing --- review \& editing].
\end{itemize}

The pilot DALL$\cdot$E outputs were independently coded by Author 1\ and Author 2\ with 100\% inter-annotator agreement. Author 3\ joined for the v2 program; the pilot annotation history is preserved as 2-author.
